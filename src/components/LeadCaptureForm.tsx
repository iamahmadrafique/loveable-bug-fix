  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (errors.length === 0) {
      ...
      try {
        ...
        if (emailError) {
          setServerError('There was a problem sending your confirmation email. Please try again later.');
        ...
        setServerError('There was a problem contacting the server. Please try again later.');
    ...
    <div className="w-full max-w-md mx-auto">
      <div className="bg-gradient-card p-8 rounded-2xl shadow-card border border-border backdrop-blur-sm animate-slide-up">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {serverError && (
            <p className="text-destructive text-sm animate-fade-in">{serverError}</p>
          )}
